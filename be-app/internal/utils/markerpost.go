...
func HandleMarkerPostConversion(latitudeStr, longitudeStr,
	...
	visibilityIDStr string, imageFIle *multipart.FileHeader) (float32, float32, []byte, string, int, int, error) {

	latitude, err := strconv.ParseFloat(latitudeStr, 32)

	if err != nil {
		fmt.Println("Error converting to float32 for latitude")
		return -1, -1, nil, "", -1, -1, err
	}
	...
	return float32(latitude), float32(longitude), image, descriptionStr, visibilityID, userID, nil
}
